...
impl EventManager{
    // 添加事件监听，将玩家添加到指定事件下
    pub async fn add_subscribe_listener(&self, event: Event, player: IPlayer) {
    ...
    pub async fn trigger_event(&self, event: Event) {
        let eventlist = self.eventlist.lock().await; // 获取事件列表的不可变引用
        if let Some(players) = eventlist.get(&event) {
            for player in players {
                // 处理事件，例如发送数据给玩家
                println!("Event triggered for player: {:?}", player);
            }
        }
    }

    // 获取指定事件下的所有玩家
    pub async fn get_players_for_event(&self, event: Event) -> Option<Vec<IPlayer>> {
